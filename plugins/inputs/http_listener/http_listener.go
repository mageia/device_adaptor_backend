...
func (h *HTTPListener) SelfCheck() device_adaptor.Quality {
...
func (h *HTTPListener) CheckGather(acc device_adaptor.Accumulator) (err error) {
	if len(h.parsers) == 0 {
	...
	if len(h.resultMap) > 0 {
		acc.AddFields(h.Name(), h.resultMap, nil, device_adaptor.QualityGood, time.Now())
	}
...
func init() {
	inputs.Add("http_listener", func() device_adaptor.Input {
		return &HTTPListener{
